...
#include "ldpcDec.h"
#include <string>
void readData(std::vector<ap_int<8> > llr,std::vector<std::vector<ap_int<8> > > minfo,std::vector<std::vector<ap_int<8> > > blockllr){
	for(int i=0;i<24;i++){
		for(int j=0;j<BL;j++){
#pragma HLS PIPELINE
...
void updateMinfo(std::vector<ap_int<8> > llr, std::vector<std::vector<ap_int<8> > > minfo,std::vector<std::vector<ap_int<8> > > blockllr,bool blockdout[24][512]){
...
void ldpcDec(std::vector<ap_int<8> > llr,bool output[K]){
#pragma HLS INTERFACE axis register both port=llr
#pragma HLS INTERFACE axis register both port=output
	std::vector<std::vector<ap_int<8> > > minfo;
	std::vector<std::vector<ap_int<8> > > blockllr;
...
inline void colUpdate3(bool r[512],std::vector<ap_int<8> > l,std::vector<ap_int<8> > a,std::vector<ap_int<8> > b,std::vector<ap_int<8> > c){
	for(int i=0;i<BL;i++){
#pragma HLS PIPELINE
		std::vector<ap_int<8> > t;
		t[0] = a[i];
		t[1] = b[i];
		t[2] = c[i];
		ap_int<11> sum = l[i] + t[0] + t[1] + t[2];
		std::vector<ap_int<11> > s;
#pragma UNROLL
		for(int j=0;j<3;j++){
			s[j] = sum - t[j];
			if(s[j]>127)
...
inline void colUpdate2(bool r[512],std::vector<ap_int<8> > l,std::vector<ap_int<8> > a,std::vector<ap_int<8> > b){
	for(int i=0;i<BL;i++){
	#pragma HLS PIPELINE
		std::vector<ap_int<8> > t;
			t[0] = a[i];
...
inline void colUpdate6(bool r[512],std::vector<ap_int<8> > l,std::vector<ap_int<8> > a,std::vector<ap_int<8> > b,std::vector<ap_int<8> > c,std::vector<ap_int<8> > d,std::vector<ap_int<8> > e,std::vector<ap_int<8> > f){
	for(int i=0;i<BL;i++){
	#pragma HLS PIPELINE
		std::vector<ap_int<8> > t;
			t[0] = a[i];
